...
$sql_x  = "SELECT * FROM gerador_conf WHERE controle = '$Cod_2' ORDER BY COD";

$resultado_gera = @mysql_query($sql_x);

$row_gera = @mysql_fetch_array($resultado_gera);

$id_gera      = @$row_gera["controle"];
$vencto       = @$row_gera["VENCTO"];
$vencto1      = @$row_gera["VENCTO"];
$nudoc        = @$row_gera["controle"];
//$num_doc      = $linha1["COD"];
$num_doc      = @$row_gera["controle"];
$estado       = @$row_gera["UF"];
$valor        = $valor1;
...
$uf           = @$row_gera["UF"];
$adm          = @$row_gera["ADM"];
$prorr_ga     = @$row_gera["PRORROGA"];
$tipo_con     = @$row_gera["TIPO_CONT"];
$instru_c     = @$row_gera["INSTRUCAO"];
$exec         = @$row_gera["EXEC"];

if ($tipo_con == 'ASSISTENCIAL' OR $tipo_con == 'CONFEDERATIVA'){
	...
	$sql_x  = "SELECT * FROM gerador_conf WHERE controle = '$id_gera'";
	...
	$consultaz  = "SELECT * FROM instrucao WHERE Edit1 = '$instru_c'";
	...
	$sql_x  = "SELECT * FROM gerador_conf WHERE controle = '$Cod_2'";
	...
	$pdf->image("../imagens/cef.jpg",0,0,211,303);
	...
	$valorz = formata_numero($nov_valor2,10,0);
		    ...
		    $cod_ent     = $Edit16;
		    $cod_cnae    = 8;  // 9
		    $tipo_ent    = 2; // 1 = Sindicato, 2 = Federação, 3 = Confederação, 4 = MTE/CEES 
		    $sitcs       = 77;
