...
$Edit1        = addslashes($_POST['Edit1']);
$Edit2        = addslashes($_POST['Edit2']); // Qtd Mes
$Edit3        = addslashes($_POST['Edit3']);
$Edit4        = addslashes($_POST['Edit4']);
$Edit5        = addslashes($_POST['Edit5']);
...
$data_log = date("d/m/Y");
$hora_log = date("H:i:s"); 
$even_log = "CARNE SOCIOS"."/".$cod_edif1;
...
$sql  = "SELECT * FROM socios WHERE CODP >= '". anti_sql_injection($cod_edif1) ."' and CODP <= '". anti_sql_injection($cod_edif2) ."' and CATEGORIA != 'O'";
...
$consulta6a  = "SELECT * FROM oposicao3 Where RGASSOC = '". anti_sql_injection($rg) ."'";
...
if ($cod_opo != 0){
	...
	echo  'Sócio Com Carta de OPOSIÇÃO sem emissão de Boletos '.$i_ano_data.'<br>';
	echo  'Nome do Sócio   '.$sacado.'<br>';
	echo  'Matricula Nº  '.$cod_p.'<br>';
	$conta_dor = "";	
	?>
...
$consulta7a  = "SELECT * FROM oposicao3 Where CPF = '". anti_sql_injection($cnpj) ."'";
...
$cons_mensa_7  = "SELECT * FROM aberto_soc WHERE CODP = '". anti_sql_injection($cod_p) ."'  ORDER BY ANO ASC, MES ASC";
...
while($nov_ano < $ate_nov_ano and $conta_dor < $i_ate_mes){
      ...
      if ($conta_dor > 150){
      	...
      	echo "Ocorreu um Erro Interno: Tente novamente!!! Nº erro:".$conta_dor."<br>";
...
echo 'Sócio Consta como REMIDO sem emissão de Boletos '.$i_ano_data.'<br>';
echo 'Nome do Sócio   '.$sacado;
$conta_dor = "";
?>
...
echo  'Sócio Consta como DESISTENTE sem emissão de Boletos '.$i_ano_data.'<br>';
echo  'Nome do Sócio   '.$sacado;
$conta_dor = "";	
?>
...
echo  'Sócio Consta como FALECIDO sem emissão de Boletos '.$i_ano_data.'<br>';
echo  'Nome do Sócio   '.$sacado;
$conta_dor = "";	
?>
...
echo  'Sócio Com CPF invalido Verifique !!! CPF.: '.$cnpj.'<br>';
echo  'Nome do Sócio   '.$sacado;
$conta_dor = "";	
?>
...
if (!empty($conta_dor)){
		...
		$data_log = date("d/m/Y");
		$hora_log = date("H:i:s"); 
		$even_log = "Retirada de Carne Mensalidade";
		
		$consulta_log  = "SELECT * FROM socios_historico WHERE CODP = '". anti_sql_injection($cod_p) ."' AND DATE = '". anti_sql_injection($data_log) ."'";
		...
		//ECHO "ENTREI AQUI ".$cod_p;
			...
			$cons_10 = "UPDATE socios_historico SET DATA = '$data_log',
			                                        HORA = '$hora_log'  WHERE CODP = '". anti_sql_injection($cod_p) ." $cod_p' AND DATE = '". anti_sql_injection($data_log) ."'";
...
$pdf->Text(12,10, 'Não Consta Debito referente ao ano de  '.$i_ano_data.', sem Emissão de Boletos ');
$pdf->Text(12,18, 'Sócio  '.$sacado);
