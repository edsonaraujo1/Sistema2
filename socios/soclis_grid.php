...
function keyCatcher() 
{
   ...
   if (event.keyCode == 27) 
   {
		window.location="cadsocios.php?Cod_xxx=<?php echo $id ?>";
   }
...
$data_log = date("d/m/Y");
$hora_log = date("H:i:s"); 
$even_log = "LISTA CONSULTA SOCIOS/";
...
$sql  = "SELECT * FROM socios WHERE id >= '". anti_sql_injection($Cod_2) ."' ORDER BY COD ASC";

if ($Opcao == "MATRICULA"){

   $sql  = "SELECT * FROM socios WHERE id >= '". anti_sql_injection($Procura) ."' ORDER BY COD ASC";
	
}
if ($Opcao == "NOME"){

   $sql  = "SELECT * FROM socios WHERE NOMEASSOC >= '". anti_sql_injection($Procura) ."' ORDER BY NOMEASSOC ASC";
	
}
if ($Opcao == "ENDERECO"){

   $sql  = "SELECT * FROM socios WHERE ENDRESID >= '". anti_sql_injection($Procura) ."' ORDER BY ENDRESID ASC";
	
}
if ($Opcao == "CPF"){

   $sql  = "SELECT * FROM socios WHERE CPF >= '". anti_sql_injection($Procura) ."' ORDER BY CPF ASC";
	
}
if ($Opcao == "RG"){

   $sql  = "SELECT * FROM socios WHERE RG >= '". anti_sql_injection($Procura) ."' ORDER BY RG ASC";
	...
	$id1 = $id;
      echo "
       ...
       while ($linha = @mysql_fetch_array($resultado))
       {
	    ...
	    $idx       = $linha["id"]; 
	  	$coluna1  = $linha["CODP"];
		$coluna2  = substr($linha["NOMEASSOC"],0,38);
		$coluna3  = $linha["CPF"];
		$coluna4  = $linha["RG"];
		$coluna5  = trim($linha["RUA"]);
		$coluna6  = substr($linha["ENDRESID"],0,43);
        ...
        <tr> 
	    <td valign=top><a href=cadsocios.php?Cod_xxx=$idx><b>$coluna1</b></a><td>$coluna2<td>$coluna5 $coluna6 $coluna7<td>$coluna3</td>";
     ...
     echo "
	         ...
	         <form method='POST' action='cadsocios.php?Cod_xxx=$id1'>
	         <td><input type=image name='incluir' src='../imagens/botao_voltar.PNG'></td>
	         </form>
