...
$sql  = "SELECT * FROM clube_tiete WHERE id = '$Cod_2'";
...
if (empty($id)){
	...
	exit;
			...
			$data_log = date("d/m/Y");
			$hora_log = date("H:i:s"); 
			$even_log = "IMPRESSAO DE REL.Club/".$Edit1;
...
$sql  = "SELECT * FROM clube_tiete WHERE CODIGO = '$cod_nu'"; // id = '$Cod_2'";
...
$id2          = $linha["id"];
$codigo       = trim($linha["CODIGO"]);
$matricula    = $linha["MATRICULA"];
$data         = $linha["DATA"];
$nome         = $linha["NOME"];
$endereco     = trim($linha["ENDER"]);
$bairro       = $linha["BAIRRO"];
$cep          = $linha["CEP"];
$funcao       = $linha["MATRICULA"];
$obs          = $linha["OBS"];
$letra        = trim($linha["LETRA"]);


$pdf->image("../imagens/Cracha_01.jpg",0,0,86,54);

$conf_fot = "images/".$codigo.$letra.".jpg";
$conf_bra = "../imagens/Branco.jpg";

if(file_exists($conf_fot)){

   $pdf->image($conf_fot,5,22,20,25);

}else{
	
   //$pdf->image($conf_bra,5,22,20,25);
	
}



//$pdf->image("../imagens/reg_1.jpg",4,2,15,17);

//$pdf->image("../imagens/logo_sind.jpg",68,2,15,15);

$pdf->SetFont('Arial', 'B', 10);

//$pdf->Text(21,10, "Clube de Regatas do Tiête");

$pdf->SetFont('Arial', 'B', 7);

$pdf->SetXY(3,25);
//$pdf->MultiCell(100,5, $nome, 0, 'C',0); // J Justificado

$pdf->SetFont('Arial', 'B', 11);
$pdf->SetXY(3,30);
//$pdf->MultiCell(100,5, $codigo.$letra, 0, 'C',0); // J Justificado

$pdf->SetXY(3,35);
//$pdf->MultiCell(100,5, $matricula, 0, 'C',0); // J Justificado

$pdf->SetXY(3,40);
//$pdf->MultiCell(100,5, $data, 0, 'C',0); // J Justificado
