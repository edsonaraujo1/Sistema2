...
$sql  = "SELECT * FROM recebimentos WHERE data_1 >= '$inicio_x' AND data_1 <= '$fim_x' ORDER BY str_to_date( data_pagto, '%d/%m/%Y')";
...
if (empty($id)){
	...
	exit;
			...
			$data_log = date("d/m/Y");
			$hora_log = date("H:i:s"); 
			$even_log = "IMPRESSAO DE REL.DIARIOS/".$Edit1;
...
if ($cont_linha == 0){
	...
	$pdf->SetXY(12,20);
	$pdf->MultiCell(182,5, 'Relatorio de Pagamentos Diarios', 0, 'C',0); // J Justificado
	$pdf->SetFont('Arial', 'B', 7);
	...
	$pdf->Text(28,30, 'Fornecedor');
	$pdf->Text(78,30, 'Produto');
    $pdf->SetXY(14,27);
    $pdf->MultiCell(150,5, 'Valor', 0, 'R',0); // J Justificado
	$pdf->Text(170,30, 'Data do Pagamento');
...
$pdf->Text(28,$lin, substr($fornecedor,0,27));

$pdf->Text(78,$lin, substr($produto,0,40));

$pdf->SetXY(14,$lin-3);
$pdf->MultiCell(150,5, number_format($valor,2,",","."), 0, 'R',0); // J Justificado

$pdf->Text(174,$lin, $data_pag);

$valor_fim = $valor_fim + $valor;
