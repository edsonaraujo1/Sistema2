...
class DHTML_Calendar {
    ...
    function _make_calendar($other_options = array(),$name='') {
        if ($name=='') $name="dcalendar";
        $js_options = $this->_make_js_hash(array_merge($this->calendar_options, $other_options));
        $code  = ( '<script type="text/javascript">var '.$name.'=Calendar.setup({' .
                   $js_options .
                   '});</script>' );
        return $code;
    }
    ...
    function make_input_field($cal_options = array(), $field_attributes = array()) {
        ...
        $attrstr = $this->_make_html_attr(array_merge($field_attributes,
                                                      array('id'   => $this->_field_id($id),
                                                            'type' => 'text')));
        echo '<table cellpadding="0" cellspacing="0" border="0"><tr><td><input ' . $attrstr .'/></td><td>';
        echo '<a href="#" id="'. $this->_trigger_id($id) . '">' .
            '<img align="absmiddle" border="0" src="' . $this->calendar_lib_path . 'img.gif" alt="" /></a></td></tr></table>';
    ...
    function _make_js_hash($array) {
        ...
        {
                if ($key=='flatCallback') $jstr .= ',' . $key . ':' . $val;
                else if ($key=='dateStatusFunc') $jstr .= ',' . $key . ':' . $val;                
                else
                {
