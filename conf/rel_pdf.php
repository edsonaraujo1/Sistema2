...
$sql  = "SELECT * FROM `conf` WHERE str_to_date( DAT_BAI, '%d/%m/%Y' ) >= str_to_date('$inicio','%d/%m/%Y' )";
...
//	$sql1  = "SELECT * FROM sindical WHERE EXER = '$iniciop' ORDER BY SINDCOD ASC";
	$sql1  = "SELECT * FROM `conf` WHERE str_to_date( DAT_BAI, '%d/%m/%Y' ) >= str_to_date('$inicio','%d/%m/%Y' ) AND str_to_date( DAT_BAI, '%d/%m/%Y' ) <= str_to_date('$fim','%d/%m/%Y' ) ORDER BY CONFCOD ASC";
...
//	$sql1  = "SELECT * FROM `conf` WHERE substring( VENCTO, 4,7 ) = substring('$inicio', 4,4 ) ORDER BY CONFCOD ASC";
	...
	if ($cont_linha == 0){
		...
		$pdf->MultiCell(182,5, 'Relatorio de Baixa Conf./Assit. de '.$inicio." ate ".$fim, 0, 'C',0); // J Justificado
		$pdf->SetFont('Arial', 'B', 7);
