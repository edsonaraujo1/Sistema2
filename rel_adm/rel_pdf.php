...
//	$sql1  = "SELECT * FROM `conf` WHERE substring( VENCTO, 4,7 ) = substring('$inicio', 4,4 ) ORDER BY CONFCOD ASC";
	...
	if ($cont_linha == 0){
		...
		$pdf->MultiCell(182,5, 'Relatorio Contabilidade', 0, 'C',0); // J Justificado


		$pdf->SetXY(12,24);
		$pdf->MultiCell(182,5, 'Nº de Empresas Administradas', 0, 'C',0); // J Justificado
		...
		$pdf->SetXY(14,30);
		$pdf->MultiCell(124,5, 'Bairro', 0, 'R',0); // J Justificado

		$pdf->Text(165,33, 'Fone');
		$pdf->Text(183,33, 'Empresas');
	}
	...
	$pdf->Text(25,$lin, substr($nome,0,32));
		
	$pdf->Text(75,$lin, substr($rua." ".$endereco,0,30));
	...
	$pdf->Text(130,$lin, substr($bairro,0,30));
		
	
	$pdf->Text(165,$lin, substr($cep,0,10));
		
	$pdf->Text(188,$lin, $qtd_edif);
