...
if ($recebe == 1){
    ...
    $sql  = "SELECT * FROM sindical WHERE SUBSTR(VENCTO,7,4) = '$iniciop' ORDER BY str_to_date( PAGTO, '%d/%m/%Y')";

}else{
	
    $sql  = "SELECT * FROM sind_cnpj_nao  WHERE SUBSTR(VENCIMENTO,7,4) = '$iniciop' ORDER BY str_to_date( PAGTO, '%d/%m/%Y')";
...
if (empty($id)){
	...
	exit;
			...
			$data_log = date("d/m/Y");
			$hora_log = date("H:i:s"); 
			$even_log = "IMPRESSAO DE REL.sindical/".$inicio_x;
...
if ($recebe == 1){
    ...
    $sql  = "SELECT * FROM sindical WHERE SUBSTR(VENCTO,7,4) = '$iniciop' ORDER BY SINDCOD";
...
if ($cont_linha == 0){
	...
	if ($recebe == 1){
		...
		$pdf->MultiCell(182,5, 'Relatorio de Pagamentos Sindical '.$inicio, 0, 'C',0); // J Justificado
		$pdf->SetFont('Arial', 'B', 7);
		...
		$pdf->Text(28,30, 'Nome Edificio');
		$pdf->Text(78,30, 'Endereco');
	    $pdf->SetXY(14,27);
	    $pdf->MultiCell(150,5, 'Valor', 0, 'R',0); // J Justificado
		$pdf->Text(165,30, 'Adms');
		$pdf->Text(173,30, 'Exercicio');
		
     }else{
     	
		$pdf->MultiCell(182,5, 'Relatorio de CNPJ que não Costa no Edificios - SINDICAL - '.$inicio, 0, 'C',0); // J Justificado
		$pdf->SetFont('Arial', 'B', 7);
		$pdf->Text(12,25, '_____________________________________________________________________________________________________________________________________');
		$pdf->Text(12,26, '_____________________________________________________________________________________________________________________________________');
		//               012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890
...
if ($recebe == 1){
	...
	$pdf->Text(78,$lin, substr($end_edif,0,40));
	
	$pdf->SetXY(14,$lin-3);
	$pdf->MultiCell(150,5, number_format($val_sindi,2,",","."), 0, 'R',0); // J Justificado
	
	$pdf->Text(166,$lin, $adm_edif);
	
	$pdf->Text(175,$lin, $exer);
...
if ($recebe == 1){
	...
	$pdf->Text(120,$lin+8, 'Total.:');
	
	$pdf->SetXY(14,$lin+5);
	$pdf->MultiCell(150,5, number_format($valor_fim,2,",","."), 0, 'R',0); // J Justificado
