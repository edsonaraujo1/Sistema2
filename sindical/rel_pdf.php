...
//    $sql  = "SELECT * FROM sindical WHERE SUBSTR(VENCTO,7,4) = '$iniciop' ORDER BY str_to_date( PAGTO, '%d/%m/%Y')";
    ...
    $sql  = "SELECT * FROM `sindical` WHERE str_to_date( DATA, '%d/%m/%Y' ) >= str_to_date('$inicio','%d/%m/%Y' )";


}else{
	
	$sql  = "SELECT * FROM sind_cnpj_nao  WHERE str_to_date( DATA, '%d/%m/%Y' ) >= str_to_date('$inicio','%d/%m/%Y' )";
}
...
//	$sql1  = "SELECT * FROM sindical WHERE EXER = '$iniciop' ORDER BY SINDCOD ASC";
	$sql1  = "SELECT * FROM `sindical` WHERE str_to_date( DATA, '%d/%m/%Y' ) >= str_to_date('$inicio','%d/%m/%Y' ) AND str_to_date( DATA, '%d/%m/%Y' ) <= str_to_date('$fim','%d/%m/%Y' ) ORDER BY SINDCOD ASC";
	...
	if ($cont_linha == 0){
		...
		$pdf->MultiCell(182,5, 'Relatorio de Baixa Sindical de '.$inicio." ate ".$fim, 0, 'C',0); // J Justificado
		$pdf->SetFont('Arial', 'B', 7);
		...
		$pdf->Text(175,30, 'Vencto / Exercicio');
	}
	...
	$pdf->Text(25,$lin, substr($nome_edif,0,28));
...
if ($recebe == 2){
	...
	$sql  = "SELECT * FROM sind_cnpj_nao  WHERE str_to_date( DATA, '%d/%m/%Y' ) >= str_to_date('$inicio','%d/%m/%Y' ) AND str_to_date( DATA, '%d/%m/%Y' ) <= str_to_date('$fim','%d/%m/%Y' )";
