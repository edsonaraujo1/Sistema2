...
$sql  = "SELECT * FROM socios_historico WHERE str_to_date(DATA, '%d/%m/%Y') >= '$inicio_x' AND str_to_date(DATA, '%d/%m/%Y') <= '$fim_x' ORDER BY id ASC";
...
if (empty($id)){
	...
	exit;
			...
			$data_log = date("d/m/Y");
			$hora_log = date("H:i:s"); 
			$even_log = "REL. CARNE Soc_dia/".$Edit1;
...
if ($cont_linha == 0){
	// Cabecalho do Relatorio
		...
		$pdf->MultiCell(182,5, 'Relatório Carne Emitidos de Socios', 0, 'C',0); // J Justificado
		$pdf->SetFont('Arial', 'B', 7);
...
$pdf->Text(14,$lin+8, 'Total = '.$total.' Carnes');
