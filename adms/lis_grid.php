...
if ($Opcao == "CODIGO"){
   ...
   $sql  = "SELECT * FROM adms WHERE cod >= '". anti_sql_injection($Procura) ."' ORDER BY cod ASC";
	
}
if ($Opcao == "NOME"){

   $sql  = "SELECT * FROM adms WHERE nomeadm >= '". anti_sql_injection($Procura) ."' ORDER BY nomeadm ASC";
	
}
if ($Opcao == "ENDEREÇO"){

   $sql  = "SELECT * FROM adms WHERE endadm >= '". anti_sql_injection($Procura) ."' ORDER BY endadm ASC";
	
}
if ($Opcao == "CNPJ"){

   $sql  = "SELECT * FROM adms WHERE cgc >= '". anti_sql_injection($Procura) ."' ORDER BY cgc ASC";
     ...
     <td><input type=image name='Voltar' src='../imagens/botao_voltar.PNG'></td>
      ...
      echo "
       ...
       while ($linha = @mysql_fetch_array($resultado))
       {
	    ...
	    $id       = $linha["id"]; 
	  	$coluna1  = $linha["cod"];
		$coluna2  = substr($linha["nomeadm"],0,25);
		$coluna3  = trim($linha["rua"]);
		$coluna4  = substr($linha["endadm"],0,15);
		$coluna5  = trim($linha["numero"]);
		$coluna6  = $linha["cgc"];
		$coluna7  = $linha["fone"];
		$coluna8  = $linha["nu_pred"];
        ...
        <tr> 
	    <td valign=top><a href=cadastro.php?cod_5=$coluna1><b>$coluna1</b></a><td>$coluna2 <td>$coluna3 $coluna4 $coluna5<td>$coluna6</td>";
