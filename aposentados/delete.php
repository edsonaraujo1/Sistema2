...
$consulta0  = "SELECT * FROM aposentados WHERE id = '$Cod_2'";
...
$Edit3   = trim(strtoupper(retirar_carac(@$row0["CPF"])));
$Edit4   = trim(strtoupper(retirar_carac(@$row0["RG"])));
$Edit5   = trim(strtoupper(retirar_carac(@$row0["OAB"])));
$Edit6   = trim(strtoupper(retirar_carac(@$row0["FONE"])));
$Edit7   = trim(strtoupper(retirar_carac(@$row0["CELULAR"])));
$Edit8   = trim(retirar_carac(@$row0["EMAIL"]));
$Edit9   = trim(strtoupper(retirar_carac(@$row0["ENDER"])));
$Edit10  = trim(strtoupper(retirar_carac(@$row0["BAIRRO"])));
$Edit11  = trim(strtoupper(retirar_carac(@$row0["ESTADO"])));
$Edit12  = trim(strtoupper(retirar_carac(@$row0["CEP"])));
$Edit13  = trim(retirar_carac(@$row0["OBS"]));
$Edit14  = @$row0["FOTO"];
...
$consulta1 = "INSERT INTO aposentados_excluir   (CODIGO,
											NOME,
		                                    CPF,
											RG,
											OAB,
...
// Exclui Edificio
$consulta2 = "DELETE FROM aposentados WHERE id = '$Cod_2'";

@mysql_query($consulta2, $link);


// Organiza Tabela apos a Exclusao
$sql0 = 'ALTER TABLE `aposentados` ORDER BY `CODIGO`';
$res0 = mysql_query($sql0);

$sql3 = 'ALTER TABLE `aposentados` DROP `id`';
$res3 = mysql_query($sql3);

$sql4 = 'ALTER TABLE `aposentados` ADD `id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST';
$res4 = mysql_query($sql4);


$consulta4  = "SELECT * FROM aposentados ORDER BY id ASC LIMIT 0,50";
