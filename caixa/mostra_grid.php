...
$deixar = acesso_url("GRID_CAIXA");

if ($deixar == "SIM"){


$titulo_tabela = "'MOVIMENAÇÃO - CAIXA -'";
?>
...
if ($empr == 1){
	...
	if (!empty($Cod_Edif)){
	   ...
	   $sql  = "SELECT * FROM caixa WHERE COD = '". anti_sql_injection($Cod_Edif) ."' ORDER BY str_to_date( VENCTO, '%d/%m/%Y')";
		//echo 'entrei aqui';
	}else{
		
	   $sql  = "SELECT * FROM caixa WHERE COD = '". anti_sql_injection($Procura) ."' ORDER BY str_to_date( VENCTO, '%d/%m/%Y')";
		//echo 'entrei aqui 2';
	}
	...
	if (!empty($Acao)){
	   ...
	   $sql  = "SELECT * FROM caixa WHERE COD = '". anti_sql_injection($Cod_Edif) ."' ORDER BY str_to_date( VENCTO, '%d/%m/%Y') ASC";
...
if ($Opcao == "CODIGO"){
   ...
   $sql  = "SELECT * FROM caixa WHERE COD = '". anti_sql_injection($Procura) ."' ORDER BY str_to_date( VENCTO, '%d/%m/%Y') ASC";
...
if ($Opcao == "CNPJ"){
    ...
    $sql  = "SELECT * FROM caixa WHERE CGC = '$Procura' ORDER BY str_to_date( VENCTO, '%d/%m/%Y') ASC";
}
