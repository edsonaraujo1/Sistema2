...
$consulta  = "SELECT * FROM socios_agile WHERE id >= 1 ORDER BY id ASC";
...
while ($linha = mysql_fetch_array($resultado))
{
                    ...
                    // Pesquiza Codigo no Cadastro
                    $sql  = "SELECT * FROM dublicados WHERE CODP = '$cod_ag'";	
                    ...
                    if (!empty($id_edif)){
	                    ...
	                    // Pesquiza Edificio
	                    $sqlxx  = "SELECT * FROM socios_agile2 WHERE CODP = '$edif_cgc'";	
		
		                $resultado5xx = @mysql_query($sqlxx);
	
	                    $row5xx = @mysql_fetch_array($resultado5xx);
	
	                    $id_edifxx     = @$row5xx["id"];
	                    $edif_cgcxx    = @$row5xx["CODP"];
	                    
	                    echo "Codigo achado...".$edif_cgc."<br>";
	                    ...
	                    if (empty($id_edifxx)){
							...
							$consulta_up = "INSERT INTO socios_agile2 (COD,
																NU,
																CODP,
																RGASSOC,
																CPF,
