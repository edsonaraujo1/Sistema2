...
$sql  = "SELECT * FROM saude_atendi WHERE str_to_date(data, '%d/%m/%Y') >= '$inicio_x' AND str_to_date(data, '%d/%m/%Y') <= '$fim_x' ORDER BY str_to_date( data, '%d/%m/%Y')";
...
if (empty($id)){
	...
	exit;
			...
			$data_log = date("d/m/Y");
			$hora_log = date("H:i:s"); 
			$even_log = "IMPRESSAO DE REL.Medic/".$Edit1;
...
if ($cont_linha == 0){
	...
	$pdf->SetXY(12,20);
	$pdf->MultiCell(268,5, 'Relatório Atendimento Medico Diarios', 0, 'C',0); // J Justificado
	$pdf->SetFont('Arial', 'B', 7);
