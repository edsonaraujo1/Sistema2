...
// Monta o Email com as Noticias Selecionadas
		for ($i= 0; $i < 100; $i++){
				  ...
				  // Inicio Enviar E-Mail
					...
					if ($Faz_01 == "SIM"){
						...
						$message .= "<table width='200' border='0' align='center'>
									  <tr>
									    <td><div align='center'>
										<img src='http://189.19.215.201/Web/imagens/im_contato.JPG' width='598' height='134'>
										</div></td>
									  </tr>
									 </table>";		
						$Faz_01 = "NAO";			    
					...
					$message .= "<font color='#000099' size='3' face='Arial, Verdana'><b>". $nome_not1 ."</b></font>";
							                       
					$message .= substr($texto_not1,0,300). "...<a href='http://189.19.215.201/Web/ler.php?cod_5=". $id_1 . "&entra_no_web1=at2&email=2' target=new>Leia Mais </a>";
					...
					$phpmail->Body .= " <br />";
					//$phpmail->AddAddress('edson@sindificios.com.br','Edson');
            ...
            $phpmail->Body .= "<center>Caso não queira receber esse e-mail novamente acesse aqui <a href='http://189.19.215.201/Web/enviar_boletim/nao_receber.php?ident=" . encode5t_se($e_mail_2) . "' target='new'>link.</a></center>";
            
             
            if (!empty($_POST['id_conf'])){
            	
	            //$phpmail->ClearAddresses();
            	$phpmail->AddAddress($_POST['id_conf'],'WEb Master');        // Destino
            	$phpmail->AddReplyto('edsonaraujo1@hotmail.com', 'Web master');
	  			$send = $phpmail->Send();
	  			if($send){
