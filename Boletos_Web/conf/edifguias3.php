...
$tipo_w = trim($tipo."/Internet");

if(!empty($CNPJ)){
   	
	// Abrir tabela Empresa
	
	$consulta2  = "SELECT * FROM edificios WHERE CGC = '". anti_sql_injection($iCNPJ) ."'";
    ...
    if (!empty($_EDI_id)){
		...
		$consulta2z  = "Select * from ABERTO WHERE CONFCOD = '". anti_sql_injection($nudoc) ."' AND DESCRICAO = '". anti_sql_injection($tipo_w) ."' AND VENCTO = '". anti_sql_injection($vencto) ."'";
		
		$resultado2z = @mysql_query($consulta2z);
		
		$row2z = @mysql_fetch_array($resultado2z);
		
		$resu_1   = @$row2z['CONFCOD'];
		$qtd_aber = @$row2z['QTD'];
		...
		$consulta3z = "UPDATE aberto   SET QTD     = '". anti_sql_injection($qtd_aber) ."' WHERE confcod = '". anti_sql_injection($nudoc) ."' AND descricao = '". anti_sql_injection($tipo_w) ."' AND vencto = '". anti_sql_injection($vencto) ."'";
		
		@mysql_query($consulta3z, $link);
		...
		// Inclui no Aberto
		$consulta3z1 = "INSERT INTO aberto (CONFCOD,
		                                  DATA,
										  DESCRICAO,
										  PAGTO,
										  VENCTO,
	   ...
	   echo '<center><font color="#FF0000" size="4">Número do Documento ou CNPJ nao cadastrados!!!</font>';
	...
	$consulta3  = "SELECT * FROM iedificios WHERE COD = '". anti_sql_injection($inudoc) ."'";
	...
	$_EDI_id     = @$row3["id"];
	$nudoc       = @$row3["COD"];
	$sacado      = retirar_carac(@$row3["COND"])." ".retirar_carac(@$row3["NOME"]);
	$endereco    = retirar_carac(@$row3["RUA"])." ".retirar_carac(@$row3["ENDERECO"]).",".retirar_carac(@$row3["NUMERO"]);
	$cep         = @$row3["CEP"];
	$_EDI2       = @$row3["ADM"];
	$CNPJ        = @$row3["CGC"];
	$estado      = @$row3["UF"];
	$cidade      = retirar_carac(@$row3["CIDADE"]);
...
//echo $cep."<br>";
    ...
    if (!empty($_EDI_id)){
	   ...
	   echo '<center><font color="#FF0000" size="4">Número do Documento nao cadastrados!!!</font>';
	   echo '<center><font color="#FF0000" size="4">Caso este numero não seja o fornecido pela entidade entre em</font>';
	   echo '<center><font color="#FF0000" size="4">contato com o Sindificios para cadastramento do mesmo</font>';
	   echo '<center><font color="#FF0000" size="4">Obrigado......</font>';
	   $faz_2=1;
...
$dadosboleto["data_vencimento"] = $vencto; // Data de Vencimento do Boleto
...
$Edit2      = @$row['Edit2'];  // Nome da entidade
$Edit3      = @$row['Edit3'];  // codigo sindical
$Edit12	    = @$row["Edit12"]; // apos Vencimento
$Edit13	    = @$row["Edit13"]; // Agencia
$Edit14	    = @$row["Edit14"]; // Digito
$Edit15	    = @$row["Edit15"]; // Carteira
$Edit16	    = @$row["Edit16"]; // conta
$Edit17	    = @$row["Edit17"]; // digito

if (empty($Edit2)){
	
	$consultat  = "SELECT * FROM instrucao WHERE Edit1 = '3'";
	
	$resultadot = @mysql_query($consultat);
	
	$rowt = @mysql_fetch_array($resultadot);
	
	$Edit2      = @$rowt['Edit2'];  // Nome da entidade
	$Edit3      = @$rowt['Edit3'];  // codigo sindical
	$Edit12	    = @$rowt["Edit12"]; // apos Vencimento
	$Edit13	    = @$rowt["Edit13"]; // Agencia
	$Edit14	    = @$rowt["Edit14"]; // Digito
	$Edit15	    = @$rowt["Edit15"]; // Carteira
	$Edit16	    = @$rowt["Edit16"]; // conta
	$Edit17	    = @$rowt["Edit17"]; // digito
...
$consulta2  = "Select * from ABERTO WHERE CONFCOD = '". anti_sql_injection($nudoc) ."' AND DESCRICAO = '". anti_sql_injection($des_aberto) ."' AND VENCTO = '". anti_sql_injection($vencto) ."'";
...
$consulta3 = "UPDATE aberto   SET QTD     = '". anti_sql_injection($qtd_aber) ."' WHERE confcod = '". anti_sql_injection($nudoc) ."' AND descricao = '". anti_sql_injection($des_aberto) ."' AND vencto = '". anti_sql_injection($vencto) ."'";
...
<table cellspacing=0 cellpadding=0 border=0>
	...
	<tr>
		<td class=cp valign=top> CEP: <?=cep;?> - <?=cidade;?> - <?=estado;?></td>
	</tr>
</table>
<table cellspacing=0 cellpadding=0 border=0>
	<tr>
