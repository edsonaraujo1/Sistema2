...
$inicio      = addslashes($_POST['Edit1']);
$fim         = addslashes($_POST['Edit2']);
$recebe      = addslashes($_POST['recerber']);
...
$sql  = "SELECT * FROM `oposicao3` WHERE CPF != '0' ORDER BY NOMEASSOC ASC";
...
if (empty($id)){
    ...
    -->
     ...
     <tr>
     <td>
     <font face=arial><b>*** Informacao Não Encontrada !!! ***</b>
     <table align=center>
	...
	exit;
			...
			$data_log = date("d/m/Y");
			$hora_log = date("H:i:s"); 
			$even_log = "IMPRESSAO DE REL.Oposicao/".$inicio_x;
...
//	$sql1  = "SELECT * FROM sindical WHERE EXER = '$iniciop' ORDER BY SINDCOD ASC";
	$sql1  = "SELECT * FROM `oposicao3` WHERE CPF != '0'  ORDER BY NOMEASSOC ASC";
	...
	$codigo       = $linha["id"];
	$rg_cod       = $linha["RGASSOC"];
	$cpf_cod      = $linha["CPF"];
	$nome_cod     = $linha["NOMEASSOC"];
	$end_cod      = $linha["ENDRESID"];
	$edif_cod     = $linha["CODEDIF"];
	$adm_cod      = $linha["ADMS"];
	$data         = $linha["DATINSC"];
	$matricula    = $linha["CODP"];
	if (empty($matricula)){
	...
	$consulta_cgc  = "SELECT * FROM edificios WHERE COD = '". anti_sql_injection($edif_cod) ."'";
	...
	if ($cont_linha == 0){
		...
		$pdf->MultiCell(182,5, 'Relatorio de Carta de Oposição do ano de '.substr($inicio,6,4), 0, 'C',0); // J Justificado
		$pdf->SetFont('Arial', 'B', 7);
		...
		$pdf->SetFont('Arial', '', 7);
		$pdf->Text(12,30,  'R.G.');
		$pdf->Text(32,30,  'C.P.F.');
		$pdf->Text(53,30,  'Nome');
		$pdf->Text(105,30, 'Endereço');
		$pdf->Text(155,30, 'Matricula');
		$pdf->Text(173,30, 'Edif');
		$pdf->Text(183,30, 'Adms');
	}
	...
	$pdf->SetFont('Arial', '', 7);
	$pdf->Text(12,$lin, $rg_cod);
		
		
	$pdf->Text(32,$lin, $cpf_cod);
		
	$pdf->Text(53,$lin, substr($nome_cod,0,30));
		
		
	$pdf->Text(105,$lin, substr($end_cod,0,30));
		
	$pdf->Text(157,$lin, $matricula);
	
	$pdf->Text(172,$lin, $edif_cod);

	$pdf->Text(185,$lin, $adm_edif);
