...
$sql  = "SELECT * FROM odontologico WHERE str_to_date(data, '%d/%m/%Y') >= '$inicio_x' AND str_to_date(data, '%d/%m/%Y') <= '$fim_x' ORDER BY str_to_date( data, '%d/%m/%Y')";
...
if (empty($id)){
	...
	exit;
			...
			$data_log = date("d/m/Y");
			$hora_log = date("H:i:s"); 
			$even_log = "IMPRESSAO DE REL.Posent/".$Edit1;
...
$codigo       = $linha["matricula"];
$nome         = $linha["titular"];
$endereco     = trim($linha["dependente"]);
$bairro       = $linha["dentista"];
$cep          = $linha["procedimento"];
$funcao       = $linha["data"];
$obs          = $linha["hora"];
...
if ($cont_linha == 0){
	...
	$pdf->SetXY(12,20);
	$pdf->MultiCell(268,5, 'Relatório Atendimento Odontologico Diarios', 0, 'C',0); // J Justificado
	$pdf->SetFont('Arial', 'B', 7);
	...
	$pdf->SetFont('Arial', '', 7);
	$pdf->Text(14,30,  'Matricula');
	$pdf->Text(28,30,  'Nome');
	$pdf->Text(88,30,  'Dependente');
	$pdf->Text(160,30, 'Dentista');
	$pdf->Text(200,30, 'Procedimentos');
	$pdf->Text(235,30, 'Data');
	$pdf->Text(250,30, 'Hora');
...
$pdf->Text(88,$lin, $endereco);

$pdf->Text(160,$lin, $bairro);

$pdf->Text(200,$lin, $cep);

$pdf->Text(235,$lin, $funcao);

$pdf->Text(250,$lin, substr($obs,0,10));
...
$pdf->Text(14,$lin+8, 'Total do Dia.: '.$inicio.' até '.$fim.' =   '.$total.' Atendimentos');
