...
// Primeiro Bloco
		...
		while($linhaa = @mysql_fetch_array($resultado1a)) {
			...
			if ($t_moe_cart == 'T'){
				...
				$total_carta = $total_carta + $vlr_tot;
				
			}else{
				
				$total_dinhea = $total_dinhea + $vlr_tot;
		...
		$pdf->SetXY(12,$lin+10);
		$pdf->MultiCell(35,5, 'GUERRINI', 1, 'L',0); // J Justificado
		...
		while($linhab = @mysql_fetch_array($resultado1b)) {
			...
			if ($t_moe_cart == 'T'){
				...
				$total_cartb = $total_cartb + $vlr_tot;
			}else{
				
				$total_dinheb = $total_dinheb + $vlr_tot;
		...
		while($linhac = @mysql_fetch_array($resultado1c)) {
			...
			$t_moe_cart  = $linhac["T_MOEDA"];
	
            
			if ($t_moe_cart == 'T'){
				
				$total_cartc = $total_cartc + $vlr_tot;
			}else{
				
				$total_dinhec = $total_dinhec + $vlr_tot;
		...
		while($linhad = @mysql_fetch_array($resultado1d)) {
			...
			if ($t_moe_cart1 == 'T'){
				...
				$total_cart4b = $total_cart4b + $vlr_tot;
				
			}else{
				
				$total_dinhei4b = $total_dinhei4b + $vlr_tot;
			}
		...
		while($linhae = @mysql_fetch_array($resultado1e)) {
			...
			if ($t_moe_cart == 'T'){
				...
				$total_carte = $total_carte + $vlr_tot;
			}else{
				
				$total_dinhee = $total_dinhee + $vlr_tot;
		...
		while($linhaf = @mysql_fetch_array($resultado1f)) {
			...
			$t_moe_cart  = $linhaf["T_MOEDA"];
	
			if ($t_moe_cart == 'T'){
				
				$total_cartf = $total_cartf + $vlr_tot;
			}else{
				
				$total_dinhef = $total_dinhef + $vlr_tot;
		...
		while($linhag = @mysql_fetch_array($resultado1g)) {
			...
			$t_moe_cart  = $linhag["T_MOEDA"];
	
			if ($t_moe_cart == 'T'){
				
				$total_cartg = $total_cartg + $vlr_tot;
			}else{
				
				$total_dinheg = $total_dinheg + $vlr_tot;
